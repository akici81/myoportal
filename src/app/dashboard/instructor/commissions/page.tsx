...
export default async function InstructorCommissionsPage() {
  ...
  const getRoleBadge = (role: string) => {
    switch(role) {
      ...
      default: return <span className="inline-flex items-center rounded-md bg-gray-500/10 px-2 py-1 text-xs font-medium text-muted ring-1 ring-inset ring-gray-500/20">Üye</span>
    }
  }
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {!myCommissions || myCommissions.length === 0 ? (
          <div className="col-span-full rounded-2xl border border-white/5 card/30 p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-default">Aktif Görev Yok</h3>
            <p className="mt-1 text-sm text-muted">Şu anda atandığınız aktif bir komisyon görevi bulunmamaktadır.</p>
          </div>
        ) : (
          myCommissions.map((member: any) => (
            <div key={member.id} className="relative rounded-2xl border border-white/5 card/30 p-6 transition-all hover:border-cyan-500/30">
              ...
              <div className="mt-4">
                <h3 className="text-lg font-semibold text-default">{member.commission.name}</h3>
              ...
              <div className="mt-6 border-t border-white/5 pt-4">
                {member.commission.drive_folder_url ? (
                   <a target="_blank" rel="noopener noreferrer" href={member.commission.drive_folder_url} className="inline-flex w-full items-center justify-center rounded-xl card px-4 py-2 text-sm font-medium text-muted transition-colors hover:card hover:text-default border border-white/5" style={{ color: 'var(--text)' }}>
