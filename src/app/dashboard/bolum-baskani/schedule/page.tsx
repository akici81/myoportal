...
import { AlertTriangle, CheckCircle, Sparkles, CalendarDays, MousePointerClick, Clock, X } from 'lucide-react'
...
export default function BolumBaskaniSchedulePage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      if (finalDepartmentId) {
        // Load programs for only this department
        ...
        // Load instructors for only this department
  ...
  if (initialLoading) {
    return <div className="p-20 flex justify-center"><div className="w-10 h-10 border-4 border-cyan-500 border-t-transparent rounded-full animate-spin"></div></div>
  }
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-gray-400 max-w-2xl font-medium">
              Ait olduğunuz bölümdeki ({department?.name}) programların ders akışlarını hızlıca planlayın.
            </p>
          </div>
          {entries.length > 0 && (
        ...
        <div className="glass-card p-5 rounded-2xl border border-gray-800/60 shadow-xl relative z-20">
          ...
          {!selectedPeriod && periods.length === 0 && (
            <div className="mt-4 p-4 rounded-xl flex items-start gap-3 bg-amber-500/10 border border-amber-500/30 text-sm text-amber-200 font-medium">
              ...
              <p>Sistemde aktif bir Akademik Dönem (Academic Period) bulunamadı.</p>
            </div>
          )}
        </div>
