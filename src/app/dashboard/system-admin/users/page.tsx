        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button 
              ...
              disabled={syncing}
              className="px-4 py-3 rounded-xl bg-gray-800 border border-gray-700 text-gray-300 hover:text-white hover:bg-gray-700 transition flex items-center justify-center gap-2"
            >
      ...
      <div className="card overflow-hidden border border-gray-800/60 rounded-xl">
        ...
        <div className="p-4 border-b border-gray-800/60 bg-gray-900/50 flex items-center justify-between">
          <div className="relative w-full max-w-sm">
            ...
            <input 
              className="w-full bg-gray-800/50 border border-gray-700/50 rounded-lg pl-10 pr-4 py-2 text-sm text-gray-200 placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-500/50 transition-all font-medium" 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-gray-800/40 text-xs uppercase text-gray-400 border-b border-gray-800">
              <tr>
              ...
              {loading ? (
                Array.from({length: 5}).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    <td className="px-6 py-4"><div className="h-10 bg-gray-800/50 rounded-lg w-48" /></td>
                    <td className="px-6 py-4"><div className="h-6 bg-gray-800/50 rounded-full w-24" /></td>
                    <td className="px-6 py-4"><div className="h-4 bg-gray-800/50 rounded w-32" /></td>
                    <td className="px-6 py-4"><div className="h-6 bg-gray-800/50 rounded-full w-16" /></td>
                    <td className="px-6 py-4"><div className="h-8 bg-gray-800/50 rounded w-8 ml-auto" /></td>
                  </tr>
                ))
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    <div className="inline-flex items-center justify-center p-4 bg-gray-800/30 rounded-full mb-3">
            ...
            <form onSubmit={createUser} className="space-y-4">
              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-4">
                  ...
                  <input className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors" placeholder="Dr., Prof." value={newUser.title} onChange={e => setNewUser(u => ({...u, title: e.target.value}))} />
                </div>
                <div className="col-span-8">
                  <label className="text-xs font-medium text-gray-400 mb-1.5 block">Ad Soyad <span className="text-cyan-400">*</span></label>
                  <input className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors" placeholder="Ahmet Yılmaz" required value={newUser.full_name} onChange={e => setNewUser(u => ({...u, full_name: e.target.value}))} />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" required className="w-full bg-gray-900/50 border border-gray-700 rounded-lg pl-9 pr-3 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors" placeholder="ahmet@rumeli.edu.tr" value={newUser.email} onChange={e => setNewUser(u => ({...u, email: e.target.value}))} />
                </div>
              </div>
              
              <div>
                <label className="text-xs font-medium text-gray-400 mb-1.5 block">Geçici Şifre <span className="text-cyan-400">*</span></label>
                <input type="password" required className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors" placeholder="En az 8 karakter" value={newUser.password} onChange={e => setNewUser(u => ({...u, password: e.target.value}))} />
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-medium text-gray-400 mb-1.5 block">Yetki Rolü <span className="text-cyan-400">*</span></label>
                  <select className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors form-select appearance-none" value={newUser.role} onChange={e => setNewUser(u => ({...u, role: e.target.value as UserRole}))}>
                ...
                <div>
                  ...
                  <select className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors form-select appearance-none" value={newUser.department_id} onChange={e => setNewUser(u => ({...u, department_id: e.target.value}))}>
              ...
              <div className="pt-4 flex gap-3">
                <button type="button" onClick={() => setShowNew(false)} className="px-4 py-2.5 rounded-lg font-medium text-sm text-gray-400 bg-gray-800 hover:bg-gray-700 hover:text-white transition flex-1">İptal</button>
          ...
          <div className="card w-full max-w-lg p-6 rounded-2xl border border-gray-700/50 shadow-2xl relative">
            ...
            <form onSubmit={saveEdit} className="space-y-4">
              <div className="grid grid-cols-12 gap-4">
                <div className="col-span-4">
                  <label className="text-xs font-medium text-gray-400 mb-1.5 block">Unvan</label>
                  <input className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors" value={editUser.title ?? ''} onChange={e=>setEditUser((u:any)=>({...u,title:e.target.value}))} />
                </div>
                <div className="col-span-8">
                  <label className="text-xs font-medium text-gray-400 mb-1.5 block">Ad Soyad <span className="text-cyan-400">*</span></label>
                  <input className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors" required value={editUser.full_name ?? ''} onChange={e=>setEditUser((u:any)=>({...u,full_name:e.target.value}))} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input className="w-full font-mono bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors" value={editUser.username ?? ''} onChange={e=>setEditUser((u:any)=>({...u,username:e.target.value.toLowerCase()}))} />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-400 mb-1.5 block">Telefon</label>
                  <div className="relative">
                    <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" />
                    <input className="w-full bg-gray-900/50 border border-gray-700 rounded-lg pl-9 pr-3 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors" value={editUser.phone ?? ''} onChange={e=>setEditUser((u:any)=>({...u,phone:e.target.value}))} />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors form-select appearance-none" value={editUser.role ?? 'instructor'} onChange={e=>setEditUser((u:any)=>({...u,role:e.target.value}))}>
                    {(Object.keys(ROLE_META) as UserRole[]).map(r => (
                      <option key={r} value={r}>{ROLE_META[r].label}</option>
                    ))}
                  </select>
                ...
                <div>
                  ...
                  <select className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors form-select appearance-none" value={editUser.department_id ?? ''} onChange={e=>setEditUser((u:any)=>({...u,department_id:e.target.value||null}))}>
                    <option value="">— Genel Merkez / Yok —</option>
                    {depts.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}
                  </select>
                </div>
