    ...
    <div className="space-y-6 animate-in">
      ...
      <div className="card p-4 rounded-xl flex flex-col sm:flex-row gap-4 items-center justify-between border">
        <div className="relative w-full sm:w-80">
          ...
          <input 
            className="w-full card border rounded-lg pl-10 pr-4 py-2.5 text-sm text-muted placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/30 font-medium" 
            placeholder="Program, Bölüm Ara..." 
            value={search} 
            onChange={e => setSearch(e.target.value)} 
          />
        </div>
        <div className="text-muted text-sm font-medium">Toplam <span className="text-default">{filtered.length}</span> Program</div>
      </div>
            ...
            <div key={p.id} className="card p-5 rounded-xl border flex flex-col h-full group">
               <div className="flex items-start gap-4 mb-3">
                  ...
                  <div className="flex-1">
                     <h3 className="font-bold text-muted text-base">{p.name}</h3>
               ...
               <div className="mt-auto pt-4 border-t flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                     <button onClick={() => setEditItem(p)} className="p-1.5 text-muted hover:text-default rounded-md hover:card transition"><Edit className="w-4 h-4"/></button>
                     <button onClick={() => remove(p.id)} className="p-1.5 text-muted hover:text-red-400 rounded-md hover:card transition"><Trash2 className="w-4 h-4"/></button>
                  </div>
               </div>
            </div>
         ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 animate-in">
          <div className="card w-full max-w-md p-6 rounded-2xl border shadow-2xl relative">
            ...
            <form onSubmit={save} className="space-y-4">
              <div>
                <label className="text-xs font-medium text-muted mb-1.5 block">Program Adı <span className="text-cyan-400">*</span></label>
                <input required className="w-full card border rounded-lg px-3 py-2.5 text-sm text-default focus:outline-none focus:border-cyan-500" value={editItem.name ?? ''} onChange={e => setEditItem((i: any) => ({ ...i, name: e.target.value }))} />
              </div>
              <div>
                <label className="text-xs font-medium text-muted mb-1.5 block">Kısa Kod <span className="text-cyan-400">*</span></label>
                <input required maxLength={5} className="w-full card border rounded-lg px-3 py-2.5 text-sm text-default font-mono uppercase focus:outline-none focus:border-cyan-500" value={editItem.short_code ?? ''} onChange={e => setEditItem((i: any) => ({ ...i, short_code: e.target.value }))} />
              </div>
              <div>
                <label className="text-xs font-medium text-muted mb-1.5 block">Bağlı Olduğu Bölüm <span className="text-cyan-400">*</span></label>
                <select required className="w-full card border rounded-lg px-3 py-2.5 text-sm text-default focus:outline-none focus:border-cyan-500" value={editItem.department_id ?? ''} onChange={e => setEditItem((i: any) => ({ ...i, department_id: e.target.value }))}>
              ...
              <div className="pt-2">
                <label className="inline-flex items-center cursor-pointer">
                  ...
                  <span className="ms-3 text-sm font-medium text-muted">Program Aktif</span>
                </label>
              </div>
              <div className="pt-4 flex gap-3">
                ...
                <button type="submit" disabled={saving} className="flex-1 px-4 py-2.5 rounded-lg text-sm bg-cyan-600 text-default hover:bg-cyan-500">{saving ? 'Kaydediliyor...' : 'Kaydet'}</button>
              </div>
            </form>
          </div>
        </div>
