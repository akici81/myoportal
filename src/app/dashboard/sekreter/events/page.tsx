...
export default function SekreterEventsPage() {
  ...
  return (
    <div className="space-y-6 animate-in">
      ...
      {showForm && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 animate-in"><div className="card w-full max-w-lg p-6 rounded-2xl border shadow-2xl relative"><button onClick={() => setShowForm(false)} className="absolute right-4 top-4 p-2 text-gray-400 hover:text-white rounded-lg hover:card"><X className="w-5 h-5" /></button><h3 className="text-xl font-bold text-white mb-5 flex items-center gap-2"><CalendarDays className="w-5 h-5 text-violet-400" /> Yeni Etkinlik</h3><div className="space-y-4"><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Başlık</label><input className="w-full card border rounded-lg px-3 py-2.5 text-sm text-white" value={form.title} onChange={e => setForm(f=>({...f, title: e.target.value}))} /></div><div className="grid grid-cols-2 gap-3"><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Kategori</label><select className="w-full card border rounded-lg px-3 py-2.5 text-sm text-white" value={form.category} onChange={e => setForm(f=>({...f, category: e.target.value}))}>{Object.entries(CATEGORY_MAP).map(([k,v]) => <option key={k} value={k}>{v.label}</option>)}</select></div><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Tarih</label><input type="date" className="w-full card border rounded-lg px-3 py-2.5 text-sm text-white" value={form.date} onChange={e => setForm(f=>({...f, date: e.target.value}))} /></div><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Başlangıç</label><input type="time" className="w-full card border rounded-lg px-3 py-2.5 text-sm text-white" value={form.start_time} onChange={e => setForm(f=>({...f, start_time: e.target.value}))} /></div><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Bitiş</label><input type="time" className="w-full card border rounded-lg px-3 py-2.5 text-sm text-white" value={form.end_time} onChange={e => setForm(f=>({...f, end_time: e.target.value}))} /></div></div><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Konum</label><input className="w-full card border rounded-lg px-3 py-2.5 text-sm text-white" value={form.location} onChange={e => setForm(f=>({...f, location: e.target.value}))} /></div><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Açıklama</label><textarea className="w-full card border rounded-lg px-3 py-2.5 text-sm text-white resize-none" rows={2} value={form.description} onChange={e => setForm(f=>({...f, description: e.target.value}))} /></div></div><div className="flex gap-3 mt-5"><button onClick={() => setShowForm(false)} className="flex-1 px-4 py-2.5 rounded-lg text-sm card text-gray-400">İptal</button><button onClick={saveEvent} disabled={saving || !form.title} className="flex-1 px-4 py-2.5 rounded-lg text-sm bg-violet-600 text-white hover:bg-violet-500 disabled:opacity-50">{saving ? '...' : 'Kaydet'}</button></div></div></div>)}
    </div>
  )
}
