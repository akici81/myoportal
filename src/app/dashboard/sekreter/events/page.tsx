    ...
    <div className="space-y-6 animate-in">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-violet-900/40 via-purple-900/20 to-gray-900 p-8 border border-violet-800/30">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-purple-400">Etkinlik Takvimi</h1>
      ...
      <div className="glass-card p-4 rounded-xl border border-gray-800/60 flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-1">
          <button onClick={() => setCurrent(d => new Date(d.getFullYear(), d.getMonth()-1, 1))} className="p-2 rounded-lg hover:bg-gray-800 text-gray-500 hover:text-white transition"><ChevronLeft className="w-4 h-4" /></button>
          ...
          <button onClick={() => setCurrent(d => new Date(d.getFullYear(), d.getMonth()+1, 1))} className="p-2 rounded-lg hover:bg-gray-800 text-gray-500 hover:text-white transition"><ChevronRight className="w-4 h-4" /></button>
          <button onClick={() => setCurrent(new Date())} className="ml-1 px-3 py-1.5 text-xs rounded-lg border border-gray-700 text-gray-400 hover:text-white">Bugün</button>
        </div>
        ...
        <div className="ml-auto flex rounded-lg overflow-hidden border border-gray-700">
          ...
          <button onClick={() => setView('list')} className={`px-3 py-2 text-xs flex items-center gap-1.5 ${view === 'list' ? 'bg-gray-800 text-white' : 'text-gray-500'}`}><List className="w-3.5 h-3.5" /> Liste</button>
        </div>
      </div>

      {view === 'month' && (
        <div className="glass-card overflow-hidden rounded-xl border border-gray-800/60">
          <div className="grid grid-cols-7 border-b border-gray-800" style={{ background: 'rgba(0,0,0,0.3)' }}>{DAYS_SHORT.map(d => (<div key={d} className="py-2.5 text-center text-xs font-semibold text-gray-500">{d}</div>))}</div>
          <div className="grid grid-cols-7">
            {calendarDays.map((date, idx) => {
              const iso = date?.toISOString().split('T')[0]
              const isToday = iso === today
              const dayEv = date ? eventsOnDate(date) : []
              return (<div key={idx} className={`min-h-[100px] p-1.5 border-b border-r border-gray-800/40 ${date ? '' : 'opacity-0'} ${isToday ? 'bg-violet-500/5' : ''}`}>
      ...
      {view === 'list' && (<div className="space-y-3">{upcoming.length === 0 ? (<div className="glass-card py-16 text-center rounded-xl"><CalendarDays className="w-10 h-10 mx-auto mb-3 text-gray-600" /><p className="text-white font-medium">Yaklaşan etkinlik yok</p></div>) : upcoming.map(ev => { const d = new Date(ev.date); const cat = CATEGORY_MAP[ev.category] ?? CATEGORY_MAP.diger; return (<div key={ev.id} className="glass-card p-4 rounded-xl border border-gray-700/50 cursor-pointer hover:border-gray-600 flex items-start gap-4" onClick={() => setSelected(ev)}><div className="w-12 text-center flex-shrink-0"><div className="text-xl font-bold text-white">{d.getDate()}</div><div className="text-[10px] font-semibold uppercase" style={{ color: cat.color }}>{MONTHS_TR[d.getMonth()].slice(0,3)}</div></div><div className="flex-1 min-w-0"><div className="flex items-start justify-between gap-2"><h4 className="font-semibold text-white truncate">{ev.title}</h4><span className="text-[10px] px-2 py-0.5 rounded-full font-semibold" style={{ background: cat.bg, color: cat.color }}>{cat.label}</span></div><div className="flex gap-3 mt-1.5 text-xs text-gray-400"><span className="flex items-center gap-1"><Clock className="w-3 h-3" />{ev.start_time}</span>{ev.location && <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{ev.location}</span>}</div></div></div>) })}</div>)}

      {selected && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in"><div className="glass-card w-full max-w-md p-6 rounded-2xl border border-gray-700/50 shadow-2xl relative"><button onClick={() => setSelected(null)} className="absolute right-4 top-4 p-2 text-gray-400 hover:text-white rounded-lg hover:bg-gray-800"><X className="w-5 h-5" /></button><span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: (CATEGORY_MAP[selected.category]?.bg ?? ''), color: (CATEGORY_MAP[selected.category]?.color ?? '#64748b') }}>{CATEGORY_MAP[selected.category]?.label ?? 'Diğer'}</span><h3 className="text-lg font-bold text-white mt-3 mb-4">{selected.title}</h3><div className="space-y-2 text-sm text-gray-400"><div className="flex items-center gap-2"><CalendarDays className="w-4 h-4" />{new Date(selected.date).toLocaleDateString('tr-TR', { weekday:'long', day:'numeric', month:'long', year:'numeric' })}</div><div className="flex items-center gap-2"><Clock className="w-4 h-4" />{selected.start_time}{selected.end_time && ` – ${selected.end_time}`}</div>{selected.location && <div className="flex items-center gap-2"><MapPin className="w-4 h-4" />{selected.location}</div>}{selected.description && <p className="pt-2 border-t border-gray-800">{selected.description}</p>}</div><button onClick={() => deleteEvent(selected.id)} className="w-full mt-4 py-2 rounded-lg text-sm bg-red-600/20 text-red-400 hover:bg-red-600/30 flex items-center justify-center gap-2"><Trash2 className="w-4 h-4" /> Sil</button></div></div>)}

      {showForm && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in"><div className="glass-card w-full max-w-lg p-6 rounded-2xl border border-gray-700/50 shadow-2xl relative"><button onClick={() => setShowForm(false)} className="absolute right-4 top-4 p-2 text-gray-400 hover:text-white rounded-lg hover:bg-gray-800"><X className="w-5 h-5" /></button><h3 className="text-xl font-bold text-white mb-5 flex items-center gap-2"><CalendarDays className="w-5 h-5 text-violet-400" /> Yeni Etkinlik</h3><div className="space-y-4"><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Başlık</label><input className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white" value={form.title} onChange={e => setForm(f=>({...f, title: e.target.value}))} /></div><div className="grid grid-cols-2 gap-3"><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Kategori</label><select className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white" value={form.category} onChange={e => setForm(f=>({...f, category: e.target.value}))}>{Object.entries(CATEGORY_MAP).map(([k,v]) => <option key={k} value={k}>{v.label}</option>)}</select></div><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Tarih</label><input type="date" className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white" value={form.date} onChange={e => setForm(f=>({...f, date: e.target.value}))} /></div><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Başlangıç</label><input type="time" className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white" value={form.start_time} onChange={e => setForm(f=>({...f, start_time: e.target.value}))} /></div><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Bitiş</label><input type="time" className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white" value={form.end_time} onChange={e => setForm(f=>({...f, end_time: e.target.value}))} /></div></div><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Konum</label><input className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white" value={form.location} onChange={e => setForm(f=>({...f, location: e.target.value}))} /></div><div><label className="text-xs font-medium text-gray-400 mb-1.5 block">Açıklama</label><textarea className="w-full bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white resize-none" rows={2} value={form.description} onChange={e => setForm(f=>({...f, description: e.target.value}))} /></div></div><div className="flex gap-3 mt-5"><button onClick={() => setShowForm(false)} className="flex-1 px-4 py-2.5 rounded-lg text-sm bg-gray-800 text-gray-400">İptal</button><button onClick={saveEvent} disabled={saving || !form.title} className="flex-1 px-4 py-2.5 rounded-lg text-sm bg-violet-600 text-white hover:bg-violet-500 disabled:opacity-50">{saving ? '...' : 'Kaydet'}</button></div></div></div>)}
    </div>
  )
}
